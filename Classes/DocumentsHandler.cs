...
namespace ITDocumentation
{
    public class DocumentsHandler
    {
        ...
        public bool saveDocuments(List<Document> documents, string state, string parent)
        {
            foreach (var document in documents)
            {
                ...
                if (!dbContext.Document.Any(d => d.Name == document.Name))
                {
                    dbContext.Document.Add(document);
                    dbContext.SaveChanges();
                    /**Document lastInserted = dbContext.Document.OrderByDescending(d => d.ID).FirstOrDefault()!;
                    DocumentTags documentTags = new DocumentTags();
